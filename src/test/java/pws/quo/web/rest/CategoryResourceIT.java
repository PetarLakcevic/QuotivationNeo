...
class CategoryResourceIT {
    ...
    @Test
    @Transactional
    void getAllCategoriesByQuoteIsEqualToSomething() throws Exception {
        Quote quote;
        ...
        em.persist(quote);
        em.flush();
        category.addQuote(quote);
        categoryRepository.saveAndFlush(category);
        Long quoteId = quote.getId();

        // Get all the categoryList where quote equals to quoteId
        defaultCategoryShouldBeFound("quoteId.equals=" + quoteId);

        // Get all the categoryList where quote equals to (quoteId + 1)
        defaultCategoryShouldNotBeFound("quoteId.equals=" + (quoteId + 1));
    }

    @Test
    @Transactional
    void getAllCategoriesByUserAdditionalFieldsIsEqualToSomething() throws Exception {
        UserAdditionalFields userAdditionalFields;
        if (TestUtil.findAll(em, UserAdditionalFields.class).isEmpty()) {
            categoryRepository.saveAndFlush(category);
            userAdditionalFields = UserAdditionalFieldsResourceIT.createEntity(em);
        } else {
            userAdditionalFields = TestUtil.findAll(em, UserAdditionalFields.class).get(0);
        }
        em.persist(userAdditionalFields);
        em.flush();
        category.addUserAdditionalFields(userAdditionalFields);
        categoryRepository.saveAndFlush(category);
        Long userAdditionalFieldsId = userAdditionalFields.getId();

        // Get all the categoryList where userAdditionalFields equals to userAdditionalFieldsId
        defaultCategoryShouldBeFound("userAdditionalFieldsId.equals=" + userAdditionalFieldsId);

        // Get all the categoryList where userAdditionalFields equals to (userAdditionalFieldsId + 1)
        defaultCategoryShouldNotBeFound("userAdditionalFieldsId.equals=" + (userAdditionalFieldsId + 1));
    }
